...
plt.title('IDS Over Time', pad=30)
...
scatter1 = ax1.scatter(rostime1, ID1, c='black', label='ID')
...
line3, = ax1.plot(rostime1, trend_line, linestyle='--', color='red', label='ID Trend Line')

# State plotting
state_map = {0: 'Neophobic', 1: 'Neophilic'}
state_labels = [state_map[s] for s in state1]
line4, = ax2.step(rostime1, state1, where='post', linestyle='-', color='blue', label='State')

# # Annotations for ID
# id_annotations = []
# for i, txt in enumerate(ID1):
#     annot = ax1.annotate(txt, (rostime1[i], int(ID1[i]), xytext=(0, 10), textcoords="offset points", ha='center')
#     id_annotations.append(annot)

# Create checkboxes to toggle lines
rax = plt.axes([0.8, 0.5, 0.1, 0.2])
check = CheckButtons(rax, ('Probability Value', 'Activity Level', 'ID', 'ID Trend Line', 'Activity Mode'), (True, True, True, True, True))
